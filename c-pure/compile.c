...
char* compute_output_path(char const* input_file_path, char const* output_file_path) {
    ...
    if (type == FILE_DIRECTORY) {
        char const* last_fslash = strrchr(input_file_path, '/');
        char const* last_bslash = strrchr(input_file_path, '\\');
        char const* last_slash = last_fslash > last_bslash ? last_fslash : last_bslash;
        char const* input_basename = last_slash + 1;
        if (last_slash == NULL)
            input_basename = input_file_path;
        char const* extension = strrchr(input_basename, '.');
...
static char* get_obj_path(command_t* cmd, char const* path) {
...
void compile_program_native(
    ...
    switch (target) {
        case COMPILE_TARGET_C: {
            ...
            char* oflag = get_obj_path(&cc_cmd, obj_path);
